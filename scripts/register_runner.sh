...
runuser -u "$RUNNER_USER" -- env \
    ...
    sh -c 'cd "$RUNNER_DIR" && ./config.sh --url "$GITHUB_RUNNER_URL" --token "$GITHUB_RUNNER_TOKEN" --name "$RUNNER_NAME" --labels k-trader-prod --work _work --unattended --replace'
...
printf '%s\n' "PASS GitHub Actions runner ${RUNNER_VERSION} registered with label k-trader-prod"
