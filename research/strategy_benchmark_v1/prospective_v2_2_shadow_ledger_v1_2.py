...
def event_key(r):
    return (
        ...
        r.get('symbol'),
        r.get('side'),
        r.get('signal_bar_open_time'),
        r.get('setup_family_id'),
    )
...
def main():
    ...
    output = (
        ...
        if args.output
        else base / 'strategy_benchmark_v1/combined_rules/prospective_v2_2_ledger'
    )
    output.mkdir(parents=True, exist_ok=True)

    summary_paths = sorted(
        pathlib.Path(p)
        for p in glob.glob(str(base / 'v2_2_shadow_*/shadow_v1_1/summary.json'))
    )
    snapshots = []
    rejected_snapshots = []
    events_by_key = {}
    ...
    for sp in summary_paths:
        ...
        for r in events:
            ...
            if k in events_by_key:
                ...
                # Prospective causality rule: the first valid observation is
                # immutable. Later recomputation is audit evidence only and
                # must never rewrite the accepted event payload.
                continue
            events_by_key[k] = r
    ...
    report = {
        'schema_version': 'ktrader.candidate_v2_2.prospective_shadow.ledger.v1_2',
        ...
        'conflicting_duplicate_event_occurrences': conflicting_duplicates,
        'conflicting_event_key_count': len(conflicting_keys),
        ...
        'eligible_family_sizes': dict(
            sorted((k, len(v)) for k, v in family_groups.items() if k is not None)
        ),
        'clean_break_no_revisit_eligible_count': sum(
            bool(r.get('clean_break_no_revisit')) for r in eligible
        ),
        'frozen_harness_sha256_values': sorted(
            {x.get('frozen_harness_sha256') for x in snapshots if x.get('frozen_harness_sha256')}
        ),
        'protocol_sha256_values': sorted(
            {x.get('protocol_sha256') for x in snapshots if x.get('protocol_sha256')}
        ),
