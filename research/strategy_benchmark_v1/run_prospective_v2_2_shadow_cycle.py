...
async def export_panel(*, provider_id: str, panel: list[str], as_of: datetime, bundle_root: pathlib.Path, max_pages: int) -> list[dict]:
    ...
    try:
        ...
        for symbol in panel:
            ...
            out_dir = write_mtf_bundle(bundle_root, bundle)
            m = bundle.manifest
            rows.append({
                'symbol': symbol,
...
def main() -> None:
    ...
    run_root = base / f'v2_2_shadow_{run_token}'
    if run_root.exists():
    ...
    export_summary = {
        'schema_version': 'ktrader.candidate_v2_2.shadow_export.v1',
    ...
    final = {
        'schema_version': 'ktrader.candidate_v2_2.shadow_cycle.v1',
        'strategy_id': 'candidate_rule_set_v2_2',
        'holdout_opened': False,
        'production_action': False,
