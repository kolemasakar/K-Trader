...
`2026-09-16_EXTENDED_SOAK_CLOSURE_POST_CATCHUP.md`
...
- latest accepted prospective cutoff is `2026-09-16T09:45:00Z`;
- ledger state is `56` eligible observations / `43` unique families;
- accepted outcome state is `39` resolved primary families / `4` unresolved, `9` wins / `30` losses, expectancy `-0.6482720083734612R`;
- evidence status is now `DIAGNOSTIC_30_49_RESOLVED_FAMILIES`;
...
- offline resolver v1.1 preserves accepted terminal outcomes across rolling-window aging and uses persisted official funding snapshots for new resolutions;
- disk-retention dry-run planner and systemd templates are committed and validated, but host installation remains pending a bounded root action;
- no general sudo or Docker privilege was broadened;
- no rebase/force update is permitted or used.

## Latest accepted research checkpoint

- `2026-09-16_EXTENDED_SOAK_CLOSURE_POST_CATCHUP.md` — authoritative post-soak runtime/catch-up/resolver/disk-retention state.
...
Its active-soak instructions are superseded by the current checkpoint because the soak has completed.

## Recent research checkpoints

- `2026-09-16_EXTENDED_SOAK_CLOSURE_POST_CATCHUP.md` — extended-soak PASS, APT restoration, 09:45Z prospective catch-up, resolver v1.1 and dry-run retention validation;
