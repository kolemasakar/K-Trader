    ...
    async def analyze(
        ...
        *,
        ...
        for interval, count in self.config.history.interval_counts.items():
            candles = self.repository.load_recent(provider.provider_id, instrument.symbol, interval, limit=count)
            ...
            series.append(CandleSeriesSnapshot(provider.provider_id, instrument.symbol, interval, source_kind, tuple(candles)))

        setup_candles = candles_by_interval[self.config.setup_interval]
        atr14 = atr(setup_candles, period=14)
        atr5d_value = atr5d(candles_by_interval["1d"]).atr5d
        structure = build_market_structure_snapshot(
            candles_by_interval,
            timestamp=now,
            pivot_left=self.config.pivot_left,
            pivot_right=self.config.pivot_right,
            ma_method=self.config.ma_method,
            volume_window=self.config.volume_window,
            min_relative_volume=self.config.min_relative_volume,
            zone_atr_fraction=self.config.zone_atr_fraction,
        )
        traps = detect_traps(
            setup_candles,
            structure.levels.active(),
            atr14=atr14,
            min_break_atr_fraction=self.config.trap_min_break_atr_fraction,
            max_return_bars=self.config.trap_max_return_bars,
            max_confirmation_bars=self.config.trap_max_confirmation_bars,
        )
        raw_vsa = detect_vsa_events(
            setup_candles,
            baseline_window=self.config.vsa_baseline_window,
            narrow_spread_max=self.config.vsa_narrow_spread_max,
            wide_spread_min=self.config.vsa_wide_spread_min,
            low_volume_max=self.config.vsa_low_volume_max,
            high_volume_min=self.config.vsa_high_volume_min,
            stopping_volume_min=self.config.vsa_stopping_volume_min,
        )
        vsa = validate_vsa_context(
            ...
            traps=traps,
            atr14=atr14,
            max_level_distance_atr_fraction=self.config.vsa_max_level_distance_atr_fraction,
            confirmation_bars=self.config.vsa_confirmation_bars,
        )
        ...
        day_bars = [candle for candle in setup_candles if candle.open_time >= day_start and candle.open_time.date() == day_start.date()]
        day_range = build_daily_range_context(day_bars)
        setup_freshness = freshness_by_interval[self.config.setup_interval]
        decisions = tuple(
            evaluate_candidate(
                candidate,
                ...
                atr5d=atr5d_value,
                generated_at=now,
                luft_atr_fraction=self.config.luft_atr_fraction,
            )
            for candidate in candidates
        )
        if not decisions:
            decisions = (self._no_setup_decision(universe_candidate, liquidity_rank=liquidity_rank, structure=structure, atr5d_value=atr5d_value, freshness=setup_freshness, now=now),)
